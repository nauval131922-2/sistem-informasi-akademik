                    ...
                    <div class="col-sm-10">
                        <select class="form-select" id="semester" name="semester" required>
                            ...
                            @foreach ($semua_semester as $semester)
                                @if ($tahun_ajaran->semester == $semester ?? old('semester') == $semester)
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input class="form-control" type="text" placeholder="Masukkan tahun ajaran" id="tahun_ajaran"
                            name="tahun_ajaran" value="{{ $tahun_ajaran->tahun ?? old('tahun_ajaran') }}" required>
                        <div class="mt-2">
                    ...
                    <div class="col-sm-10">
                        <select class="form-select" id="statuss" name="statuss" required>
                            ...
                            @foreach ($semua_status as $status)
                                @if ($tahun_ajaran->status == $status ?? old('statuss') == $status)
...
<script>
    ...
    $('#formUbahData').on('submit', function(e) {
        ...
        var id = {{ $tahun_ajaran->id }};
        let formData = new FormData($('#formUbahData')[0]);

        $.ajax({
            url: '/data-tahun-ajaran/update/' + id,
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
